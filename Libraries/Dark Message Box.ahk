...
/************************************************************************
 * @description Apply dark theme to the built-in MsgBox and InputBox only when system is in dark mode.
 * @file Dark_MsgBox_v2_Conditional.ahk
 * @author Modified from Nikola Perovic's original
 * @date 2024/04/22
 * @version 1.1.0
 ***********************************************************************/
...
IsSystemInDarkMode() {
    try {
        ...
        ; If registry read fails, default to false (light mode)
        return false
    }
}
...
class __MsgBox
{
    static __New()
    {
        ...
        BoxEx(_this, params*)
        {
            ...
            ; Check if system is in dark mode before applying dark theme
            if (!IsSystemInDarkMode()) {
                ; System is in light mode, use native functions without dark theme
